...
vi.mock('vue-router', () => ({
  ...
  useRoute: () => ({ path: '/', params: {}, query: {}, name: 'test' })
}))
vi.mock('@/components/organisms/DashboardLayout.vue', () => ({ default: { template: '<div><slot/></div>' } }))
vi.mock('@/components/molecules/PageHeader.vue', () => ({ default: { template: '<div><slot name="actions"></slot></div>' } }))
vi.mock('@/components/atoms/BaseButton.vue', () => ({ default: { props: ['variant','loading','disabled'], template: '<button :disabled="disabled" @click="$emit(\'click\')"><slot/></button>' } }))
vi.mock('@/components/atoms/BaseModal.vue', () => ({ default: { template: '<div><slot></slot><slot name="footer"></slot></div>' } }))
vi.mock('@/composables/useSuppliers', () => ({ useSuppliers: () => ({ suppliers: [], fetchSuppliers: vi.fn(), isLoading: false }) }))
vi.mock('@/composables/useNotifications', () => ({ useNotifications: () => ({ success: vi.fn(), error: vi.fn() }) }))
...
describe('ArticleNewView create', () => {
  ...
  it('guarda artículo mínimo con nombre y rubro', async () => {
    ...
    await flushPromises()
    expect(!!saveBtn.attributes('disabled')).toBe(false)
