<template>
  <div class="checklist-card">
    ...
    <div v-if="error" class="checklist-error">
      ...
      <button class="retry-btn" @click="load" aria-label="Reintentar">Reintentar</button>
    </div>
...
.retry-btn { margin-left:auto; padding:.25rem .5rem; font-size:.75rem; border-radius:.375rem; border: var(--ps-border-width) solid var(--ps-border); background: var(--ps-card); color: var(--ps-text-primary); }
.retry-btn:hover { background: color-mix(in srgb, var(--ps-bg) 6%, var(--ps-card)); }
</style>
