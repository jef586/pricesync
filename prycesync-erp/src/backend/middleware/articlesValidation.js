...
const UpdateArticleSchema = z.object({
  ...
  gainPct: z.coerce.number().min(-100).max(100).optional().nullable().refine(v => v == null || Number.isFinite(v), { message: 'Número inválido' }).refine(v => v == null || Math.round(v * 100) === v * 100, { message: 'Máximo 2 decimales' }),
...
export const validateCreateArticle = (req, res, next) => {
  ...
  if (!parsed.success) {
    return res.status(400).json({
      error: 'VALIDATION_ERROR',
      code: 'ARTICLE_CREATE_VALIDATION_FAILED',
      details: parsed.error.errors.map(e => ({ field: e.path.join('.'), message: e.message }))
    })
  }
  req.body = parsed.data
