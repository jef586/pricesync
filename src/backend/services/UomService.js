...
export default class UomService {
  ...
  static normalizeQtyInput(uom, qty) {
    ...
    if (!UomService.isDecimal(u)) {
      // En unidades enteras, no se permiten fracciones
      if (!d.mod(1).equals(0)) {
        throw Object.assign(new Error(`Cantidad debe ser entera para UoM ${u}`), { httpCode: 400 })
      }
      return d.toDecimalPlaces(0)
    }
    return d.toDecimalPlaces(QTY_SCALE)
  }
